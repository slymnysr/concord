  ...
  useEffect(() => {
    ...
    for (const p of voice.participants()) {
      if (!names[p.userId])
        api
          .user(p.userId)
          .then((u) => setNames((n) => ({ ...n, [p.userId]: u.display_name })))
          .catch(() => {});
    }
  }, [visible, names]);
      ...
      <View style={s.root}>
          ...
          <View style={s.stageBar}>
            ...
            {!voice.amSpeaker() ? (
              <TouchableOpacity
                style={s.handBtn}
                onPress={() => {
                  voice.raiseHand(!voice.myHandRaised());
                  force();
                }}
              >
        ...
        <ScrollView contentContainerStyle={s.grid}>
          ...
          {videos.map((v) => (
            <View key={v.producerId} style={s.tile}>
              {v.stream ? (
                <RTCView streamURL={(v.stream as any).toURL()} style={s.video} objectFit="cover" />
              ) : (
                <View style={s.video} />
              )}
            ...
            <View key={p.userId} style={[s.tile, s.audioTile]}>
              <View style={s.avatarBig}>
                <Text style={s.avatarText}>
                  {(names[p.userId] ?? '?').slice(0, 1).toUpperCase()}
                </Text>
              </View>
        ...
        <View style={s.controls}>
          ...
          {Platform.OS === 'android' && (
            <Ctrl
              label="🖥️"
              active={voice.isScreenOn()}
              onPress={async () => {
                try {
                  await voice.toggleScreen();
                } catch {}
                force();
              }}
            />
