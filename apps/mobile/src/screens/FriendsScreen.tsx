...
export function FriendsScreen({ me, nav, onBack }: { me: User; nav: Nav; onBack: () => void }) {
  ...
  const filtered = friends.filter((f) =>
    ...
    : tab === 'pending' ? (f.friendship === 'pending_received' || f.friendship === 'pending_sent')
  ...
  return (
    <View style={ui.screen}>
      <ScreenHeader
        ...
        onBack={groupMode ? () => { setGroupMode(false); setSelected([]); } : onBack}
        right={groupMode
          ? <TouchableOpacity onPress={createGroup}><Text style={ui.headerBtn}>Oluştur ({selected.length})</Text></TouchableOpacity>
          : (
            <View style={{ flexDirection: 'row' }}>
              <TouchableOpacity onPress={() => { setGroupMode(true); setTab('accepted'); }}><Text style={ui.headerBtn}>Grup</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => setAdding(true)}><Text style={ui.headerBtn}>+ Ekle</Text></TouchableOpacity>
            </View>
          )}
      />
      <View style={s.tabs}>
        {([['accepted', 'Arkadaşlar'], ['pending', 'Bekleyen'], ['blocked', 'Engellenen']] as [Tab, string][]).map(([t, label]) => (
        ...
        renderItem={({ item }) => (
          ...
          >
            <View>
              ...
              {item.friendship === 'accepted' && <View style={[s.dot, { backgroundColor: statusColor(item.status) }]} />}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={s.name}>{item.display_name}</Text>
              <Text style={s.handle}>
                @{item.username}
                {item.friendship === 'pending_received' ? ' · seni ekledi' : item.friendship === 'pending_sent' ? ' · istek gönderildi' : ''}
              </Text>
            </View>
            {item.friendship === 'pending_received' && (
              <>
                <TouchableOpacity style={s.actBtn} onPress={() => act(() => api.friends.accept(item.user_id))}>
                  <Text style={{ color: colors.online, fontWeight: '700' }}>Kabul</Text>
                </TouchableOpacity>
                <TouchableOpacity style={s.actBtn} onPress={() => act(() => api.friends.remove(item.user_id))}>
            ...
            {item.friendship === 'accepted' && (groupMode ? (
              <Text style={{ color: selected.includes(item.user_id) ? colors.brand : colors.inkTertiary, fontSize: 22, paddingHorizontal: 8 }}>
                {selected.includes(item.user_id) ? '☑' : '☐'}
              </Text>
            ) : (
              <TouchableOpacity style={s.actBtn} onPress={() => Alert.alert(item.display_name, undefined, [
                { text: 'Vazgeç', style: 'cancel' },
                { text: 'Arkadaşlıktan çıkar', style: 'destructive', onPress: () => act(() => api.friends.remove(item.user_id)) },
                { text: 'Engelle', style: 'destructive', onPress: () => act(() => api.block(item.user_id)) },
              ])}>
                <Text style={{ color: colors.inkTertiary, fontSize: 20 }}>⋯</Text>
              </TouchableOpacity>
            ))}
            {item.friendship === 'blocked' && (
              <TouchableOpacity style={s.actBtn} onPress={() => act(() => api.unblock(item.user_id))}>
      ...
      <InputModal
        ...
        onSubmit={(v) => { setAdding(false); if (v.trim()) act(() => api.friends.send({ username: v.trim() })); }}
      />
    </View>
  );
}
...
const s = StyleSheet.create({
  tabs: { flexDirection: 'row', paddingHorizontal: 12, gap: 8, paddingVertical: 10, borderBottomWidth: 1, borderColor: colors.line },
