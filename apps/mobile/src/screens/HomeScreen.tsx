...
export function HomeScreen({ me, nav, onLogout }: { me: User; nav: Nav; onLogout: () => void }) {
  ...
  function channelMenu(ch: Channel) {
    Alert.alert(`#${ch.name}`, undefined, [
      ...
      { text: 'Yeniden adlandır', onPress: () => setRenameChan(ch) },
      {
        ...
        onPress: () =>
          api.channels
            .muteSettings(ch.id, { notif_level: 'nothing' })
  ...
  async function moveToFolder(guildId: string, folderId: string) {
    try {
      for (const f of folders)
        if (f.id !== folderId && f.guild_ids.includes(guildId))
          await api.folders.update(f.id, { guild_ids: f.guild_ids.filter((x) => x !== guildId) });
      const t = folders.find((f) => f.id === folderId);
      if (t && !t.guild_ids.includes(guildId))
        await api.folders.update(folderId, { guild_ids: [...t.guild_ids, guildId] });
      reloadFolders();
    } catch {}
  }
  ...
  useEffect(() => {
    ...
    const off = joinGuild(selected, (ev) => {
      if (ev === 'CHANNEL_CREATE' || ev === 'CHANNEL_UPDATE' || ev === 'CHANNEL_DELETE')
        reloadChannels();
    });
    return off;
    ...
    <View style={s.root}>
        ...
        >
          {folders.map((f) => (
            <View key={f.id} style={s.folderWrap}>
              <TouchableOpacity
                style={[s.railItem, s.folderItem]}
                onPress={() => setCollapsed((c) => ({ ...c, [f.id]: !c[f.id] }))}
              >
      ...
      <View style={s.list}>
        <View style={s.listHead}>
          <Text style={s.guildName} numberOfLines={1}>
            {guildName}
          </Text>
            ...
            ListHeaderComponent={
              <View style={s.quick}>
                ...
                <QuickRow
                  icon="🔔"
                  label="Bildirimler"
                  onPress={() => nav.push({ kind: 'notifications' })}
                />
                <QuickRow
                  icon="🔖"
                  label="Kaydedilenler"
                  onPress={() => nav.push({ kind: 'savedMessages' })}
                />
              </View>
            }
            renderItem={({ item }) => {
      ...
      >
        <Pressable style={s.modalBackdrop} onPress={() => setAddOpen(false)}>
          <Pressable style={s.modalCard} onPress={() => {}}>
            ...
            />
            <TouchableOpacity
              style={[s.modalBtn, (!inviteCode.trim() || addBusy) && { opacity: 0.4 }]}
            ...
            />
            <TouchableOpacity
              style={[s.modalBtn, (newGuildName.trim().length < 2 || addBusy) && { opacity: 0.4 }]}
              onPress={createGuild}
      ...
      <InputModal
        ...
        onSubmit={(v) => {
          ...
          if (g && v.trim())
            api.folders
              .create({ name: v.trim(), guild_ids: [g.id] })
              .then(reloadFolders)
              .catch(() => {});
        }}
      />
