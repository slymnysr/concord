...
export function LoginScreen({ onLogin }: { onLogin: (u: User) => void }) {
  ...
  async function submitForgot(value: string) {
    const step = forgot; setForgot(null);
    try {
      await setHost(host);
      if (step === 'email') {
  ...
  return (
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TextInput style={s.input} value={host} onChangeText={setHostInput} autoCapitalize="none" autoCorrect={false} placeholder="http://192.168.1.34" placeholderTextColor={colors.inkTertiary} />

        {mode === 'register' && (
          <>
            <Text style={s.label}>Kullanıcı adı</Text>
            <TextInput style={s.input} value={username} onChangeText={setUsername} autoCapitalize="none" placeholder="ornek_kullanici" placeholderTextColor={colors.inkTertiary} />
            <Text style={s.label}>Görünen ad</Text>
            <TextInput style={s.input} value={displayName} onChangeText={setDisplayName} placeholder="Adın" placeholderTextColor={colors.inkTertiary} />
          </>
        )}

        <Text style={s.label}>E-posta</Text>
        <TextInput style={s.input} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" placeholder="sen@ornek.com" placeholderTextColor={colors.inkTertiary} />
        <Text style={s.label}>Parola</Text>
        <TextInput style={s.input} value={password} onChangeText={setPassword} secureTextEntry placeholder="••••••••" placeholderTextColor={colors.inkTertiary} />

        {needTotp && (
          <>
            <Text style={s.label}>İki adımlı doğrulama kodu</Text>
            <TextInput style={s.input} value={totp} onChangeText={setTotp} keyboardType="number-pad" placeholder="123456" placeholderTextColor={colors.inkTertiary} />
          </>
        )}
        ...
        <TouchableOpacity style={[s.btn, busy && { opacity: 0.6 }]} onPress={submit} disabled={busy}>
          <Text style={s.btnText}>{busy ? 'Bekleyin…' : mode === 'login' ? 'Giriş Yap' : 'Hesap Oluştur'}</Text>
        </TouchableOpacity>

        {mode === 'login' && (
          <View style={s.linksRow}>
            <TouchableOpacity onPress={() => setForgot('email')}><Text style={s.linkSmall}>Şifreni mi unuttun?</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => setForgot('reset_token')}><Text style={s.linkSmall}>Kodu girdim</Text></TouchableOpacity>
          </View>
        )}

        <TouchableOpacity onPress={() => { setMode(mode === 'login' ? 'register' : 'login'); setNeedTotp(false); setErr(null); }}>
          <Text style={s.switch}>{mode === 'login' ? 'Hesabın yok mu? Kayıt ol' : 'Zaten hesabın var mı? Giriş yap'}</Text>
        </TouchableOpacity>
      </ScrollView>

      <InputModal
        visible={!!forgot}
        title={forgot === 'email' ? 'Şifre sıfırlama — e-posta' : forgot === 'reset_token' ? 'Sıfırlama kodu' : 'Yeni şifre'}
        placeholder={forgot === 'email' ? 'sen@ornek.com' : forgot === 'reset_token' ? 'e-postadaki kod' : '••••••••'}
...
const s = StyleSheet.create({
  ...
  input: { backgroundColor: colors.surface1, borderColor: colors.line, borderWidth: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: colors.ink, fontSize: 15 },
