...
export function MembersScreen({
  ...
  const onRefresh = async () => {
    ...
    try {
      await Promise.all([
        api.guilds.members(guildId).then(setMembers),
  ...
  return (
    <View style={ui.screen}>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setRolesFor(null)}
      >
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        run(() =>
                          has
                            ? api.guilds.unassignRole(guildId, mb.user_id, r.id)
                            : api.guilds.assignRole(guildId, mb.user_id, r.id),
                        );
                        setRolesFor({
                          ...mb,
                          role_ids: has
                            ? mb.role_ids.filter((x) => x !== r.id)
                            : [...mb.role_ids, r.id],
                        });
                      }}
                    >
                      ...
                      <Text
                        style={{ color: has ? colors.brand : colors.inkTertiary, fontSize: 18 }}
                      >
                        {has ? '✓' : '+'}
                      </Text>
                    </TouchableOpacity>
