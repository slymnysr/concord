...
type Edit =
  | { k: 'emojiName'; url: string }
  | { k: 'stickerName'; url: string }
  | { k: 'soundName'; url: string }
  | { k: 'slashName' }
  | { k: 'slashDesc'; name: string }
  | { k: 'slashResp'; name: string; desc: string }
  | { k: 'eventName' }
  | { k: 'welcomeDesc' }
  | { k: 'welcomeRules' };

export function ServerContentScreen({
  guildId,
  guildName,
  onBack,
}: {
    ...
    Alert.alert('Ne zaman?', name, [
      {
        ...
        onPress: () =>
          run(
            () =>
              api.events.create(guildId, {
                name,
                scheduled_start_at: at(3600),
  ...
  async function submitEdit(value: string) {
    ...
    setEdit(null);
    if (!e) return;
    if (e.k === 'emojiName' && value.trim())
      return run(
        () => api.emojis.create(guildId, { name: value.trim(), url: e.url }),
        'Emoji eklendi',
      );
    ...
    if (e.k === 'soundName' && value.trim())
      return run(
        () => api.sounds.create(guildId, { name: value.trim(), file_url: e.url }),
        'Ses eklendi',
      );
    ...
    if (e.k === 'slashResp' && value.trim())
      return run(
        () =>
          api.commands.create(guildId, {
            name: e.name,
            description: e.desc,
            response: value.trim(),
          }),
    ...
    if (e.k === 'welcomeDesc')
      return run(() => api.welcome.update(guildId, { description: value.trim() }));
    if (e.k === 'welcomeRules')
      return run(() => api.welcome.update(guildId, { rules_text: value.trim() }));
  }
  ...
  const addAction =
    tab === 'emojis'
      ? () => pickImageThen('emojiName')
      : tab === 'stickers'
        ? () => pickImageThen('stickerName')
        : tab === 'sounds'
          ? pickSound
          : tab === 'commands'
            ? () => setEdit({ k: 'slashName' })
    ...
    <View style={ui.screen}>
      ...
      {tab === 'emojis' && (
        <FlatList
          ...
          keyExtractor={(e) => e.id}
          numColumns={4}
          ListEmptyComponent={<Empty text="Emoji yok." />}
          renderItem={({ item }) => (
            <TouchableOpacity
              style={s.gridCell}
              onLongPress={() => run(() => api.emojis.delete(guildId, item.id))}
            >
      ...
      {tab === 'stickers' && (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={s.gridCell}
              onLongPress={() => run(() => api.stickers.delete(item.id))}
            >
      ...
      {tab === 'sounds' && (
        <FlatList
          data={sounds}
          keyExtractor={(e) => e.id}
          ListEmptyComponent={<Empty text="Ses yok." />}
          renderItem={({ item }) => (
      ...
      {tab === 'commands' && (
        <FlatList
          data={commands}
          keyExtractor={(e) => e.id}
          ListEmptyComponent={<Empty text="Komut yok." />}
          renderItem={({ item }) => (
            <View style={s.listRow}>
              <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity onPress={() => run(() => api.commands.delete(item.id))}>
                <Text style={{ color: colors.accent }}>Sil</Text>
              </TouchableOpacity>
            </View>
          )}
      ...
      {tab === 'events' && (
        <FlatList
          data={events}
          keyExtractor={(e) => e.id}
          ListEmptyComponent={<Empty text="Etkinlik yok." />}
          renderItem={({ item }) => (
            <View style={s.listRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.rowText}>{item.name}</Text>
                <Text style={s.sub}>
                  {new Date(item.scheduled_start_at).toLocaleString('tr-TR')} ·{' '}
                  {item.subscriber_count ?? 0} ilgili
                </Text>
              </View>
              <TouchableOpacity
                onPress={() =>
                  run(() =>
                    item.subscribed
                      ? api.events.unsubscribe(item.id)
                      : api.events.subscribe(item.id),
                  )
                }
              >
              ...
              <TouchableOpacity
                style={{ marginLeft: 12 }}
                onPress={() => run(() => api.events.delete(item.id))}
              >
                <Text style={{ color: colors.accent }}>Sil</Text>
              </TouchableOpacity>
            </View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
          <Row
            ...
            right={
              <TouchableOpacity
                onPress={() =>
                  run(() => api.welcome.update(guildId, { enabled: !welcome.enabled }))
                }
              >
                <Text
                  style={{
                    color: welcome.enabled ? colors.online : colors.inkTertiary,
                    fontWeight: '700',
                  }}
                >
