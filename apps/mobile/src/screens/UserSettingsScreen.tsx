...
type Edit =
  | { k: 'display_name' } | { k: 'bio' } | { k: 'pronouns' } | { k: 'avatar_color' }
  | { k: 'custom_status' } | { k: 'keywords' }
  | { k: 'pw_current' } | { k: 'pw_new'; current: string }
  | { k: 'email_new' } | { k: 'email_pw'; newEmail: string }
  | { k: 'tfa_verify' } | { k: 'tfa_disable' } | { k: 'delete_pw' };
...
export function UserSettingsScreen({ me, setMe, nav, onLogout, onBack }: {
  me: User; setMe: (u: User) => void; nav: Nav; onLogout: () => void; onBack: () => void;
}) {
  ...
  useEffect(() => {
    ...
    api.sessions.list().then((s) => setSessionCount(s.length)).catch(() => {});
  ...
  async function togglePrivacy() {
    ...
    try { await api.privacy.set(next); setPrivacy(next); } catch (e: any) { toast(e?.message ?? 'Olmadı'); }
  }

  async function start2fa() {
    if (me.mfa_enabled) { setEdit({ k: 'tfa_disable' }); return; }
    try {
      const r = await api.twofa.enable();
      Alert.alert('2FA Kurulumu', `Authenticator uygulamasına ekle:\n\n${r.secret}\n\nArdından üretilen 6 haneli kodu gir.`, [
        { text: 'Vazgeç', style: 'cancel' },
        { text: 'Kodu Gir', onPress: () => setEdit({ k: 'tfa_verify' }) },
      ]);
    } catch (e: any) { toast(e?.message ?? 'Olmadı'); }
  }

  async function githubConnect() {
    try { const r = await api.connections.githubAuthorize(); Linking.openURL(r.url).catch(() => {}); }
    catch (e: any) { toast(e?.message ?? 'GitHub bağlanamadı (sunucuda yapılandırılmamış olabilir)'); }
  }
  ...
  async function submitEdit(value: string) {
    ...
    try {
      switch (e.k) {
        case 'display_name': if (value.trim()) { await api.updateProfile({ display_name: value.trim() }); refreshMe(); } break;
        case 'bio': await api.updateProfile({ bio: value.trim() || null }); refreshMe(); break;
        case 'pronouns': await api.updateProfile({ pronouns: value.trim() }); refreshMe(); break;
        case 'avatar_color': if (/^#?[0-9a-fA-F]{6}$/.test(value.trim())) { await api.updateProfile({ avatar_color: value.trim().startsWith('#') ? value.trim() : '#' + value.trim() }); refreshMe(); } else toast('Geçerli bir renk kodu gir (örn. 00D9A6)'); break;
        case 'custom_status': await api.updateCustomStatus({ custom_status_text: value.trim() || null }); refreshMe(); break;
        case 'keywords': { const arr = value.split(',').map((s) => s.trim()).filter(Boolean); await api.keywords.set(arr); setKeywords(arr); break; }
        case 'pw_current': setEdit({ k: 'pw_new', current: value }); return;
        case 'pw_new': await api.changePassword(e.current, value); toast('Şifre değiştirildi'); break;
        case 'email_new': setEdit({ k: 'email_pw', newEmail: value }); return;
        case 'email_pw': { const r = await api.changeEmail(e.newEmail, value); toast(r.sent ? 'Doğrulama maili gönderildi' : 'İstek alındı'); break; }
        case 'tfa_verify': await api.twofa.verify(value); toast('2FA açıldı'); refreshMe(); break;
        case 'tfa_disable': await api.twofa.disable(value); toast('2FA kapatıldı'); refreshMe(); break;
        case 'delete_pw':
          await api.deleteAccount(value);
          onLogout();
          break;
      }
    } catch (err: any) { toast(err?.message ?? 'İşlem başarısız'); }
  }
  ...
  const editMeta: Record<string, { title: string; placeholder?: string; initial?: string; secure?: boolean; multiline?: boolean }> = {
    ...
    avatar_color: { title: 'Avatar rengi (hex)', initial: (me.avatar_color || '').replace('#', ''), placeholder: '00D9A6' },
    custom_status: { title: 'Özel durum', initial: me.custom_status_text, placeholder: 'Ne yapıyorsun?' },
    ...
    <View style={ui.screen}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={s.profileTop}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={s.statusInline}>
              ...
              <Text style={s.statusText}>{STATUSES.find((x) => x.v === me.status)?.label ?? me.status}</Text>
            </View>
          </View>
        </View>
        ...
        <Section title="Profil">
          ...
          <Row label="Avatar rengi" value={me.avatar_color} onPress={() => setEdit({ k: 'avatar_color' })} />
        </Section>
          ...
          <View style={s.statusRow}>
            {STATUSES.map((st) => (
              ...
              >
                ...
                <Text style={[s.statusChipText, me.status === st.v && { color: colors.ink }]}>{st.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <Row label="Özel durum" value={me.custom_status_text || '—'} onPress={() => setEdit({ k: 'custom_status' })} />
        </Section>

        <Section title="Hesap">
          <Row label="Şifre değiştir" onPress={() => setEdit({ k: 'pw_current' })} />
          <Row label="E-posta değiştir" value={me.email} onPress={() => setEdit({ k: 'email_new' })} />
          <Row
            label={me.email_verified ? 'E-posta doğrulandı' : 'E-postayı doğrula'}
            onPress={me.email_verified ? undefined : async () => { try { await api.verifyEmail(); toast('Doğrulama maili gönderildi'); } catch (e: any) { toast(e?.message ?? 'Olmadı'); } }}
        ...
        <Section title="Güvenlik">
          ...
          <Row label="Diğer oturumları kapat" onPress={async () => { try { await api.sessions.revokeOthers(); toast('Diğer oturumlar kapatıldı'); setSessionCount(1); } catch (e: any) { toast(e?.message ?? 'Olmadı'); } }} />
          <Row label={me.mfa_enabled ? 'İki adımlı doğrulama (açık)' : 'İki adımlı doğrulama'} onPress={start2fa} right={me.mfa_enabled ? <Text style={{ color: colors.online }}>✓</Text> : undefined} />
        </Section>

        <Section title="Bağlantılar">
          {connections.map((c) => (
            <Row key={c.id} label={`${c.type}: ${c.name}`} right={
              <TouchableOpacity onPress={async () => { try { await api.connections.remove(c.id); setConnections((p) => p.filter((x) => x.id !== c.id)); } catch {} }}>
        ...
        {reminders.length > 0 && (
          <Section title="Hatırlatıcılar">
            {reminders.map((r) => (
              <Row key={r.id} label={new Date(r.remind_at).toLocaleString('tr-TR')} right={
                <TouchableOpacity onPress={() => api.reminders.delete(r.id).then(() => setReminders((p) => p.filter((x) => x.id !== r.id))).catch(() => {})}>
        ...
        <Section>
          <Row label="Çıkış yap" danger onPress={() => Alert.alert('Çıkış', 'Çıkış yapılsın mı?', [{ text: 'Vazgeç', style: 'cancel' }, { text: 'Çıkış', style: 'destructive', onPress: onLogout }])} />
...
const s = StyleSheet.create({
  ...
  statusChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.surface2, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: colors.line },
