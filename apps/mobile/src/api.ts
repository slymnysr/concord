...
async function auth<T extends { access_token: string; refresh_token: string }>(
  path: string,
  body: any,
): Promise<T> {
...
export const api = {
  ...
  login: (email: string, password: string, totpCode?: string) =>
    auth<{ access_token: string; refresh_token: string; user: User }>('/auth/login', {
      email,
      password,
  ...
  sessions: {
    list: () =>
      request<Array<{ id: string; user_agent: string; created_at: string; expires_at: string }>>(
        '/users/me/sessions',
      ),
  ...
  connections: {
    list: () =>
      request<
        Array<{ id: string; type: string; name: string; verified: boolean; visible: boolean }>
      >('/users/me/connections'),
    create: (type: string, name: string) =>
      request<any>('/users/me/connections', {
        method: 'POST',
  ...
  folders: {
    list: () =>
      request<
        Array<{ id: string; name: string; color: number; position: number; guild_ids: string[] }>
      >('/users/me/folders'),
  ...
  guilds: {
    ...
    createAutomodRule: (
      ...
      request<AutomodRule>(`/guilds/${guildId}/automod-rules`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  discover: {
    list: () =>
      ...
      >('/discover/guilds'),
    join: (guildId: string) =>
      request<Guild>(`/discover/guilds/${guildId}/join`, { method: 'POST' }),
  },

  invites: {
    preview: (code: string) =>
      request<InvitePreview>(`/invites/${encodeURIComponent(code.trim())}`),
    accept: (code: string) =>
      request<Guild>(`/invites/${encodeURIComponent(code.trim())}/accept`, { method: 'POST' }),
    delete: (code: string) =>
      request<void>(`/invites/${encodeURIComponent(code)}`, { method: 'DELETE' }),
  },
  ...
  channels: {
    ...
    sendMessage: (
      ...
      request<Message>(`/channels/${channelId}/messages`, {
        ...
        body: JSON.stringify({
          content,
          replied_to_id: repliedToId || undefined,
    ...
    muteSettings: (
      channelId: string,
      input: { notif_level?: 'all' | 'mentions' | 'nothing'; mute_until_sec?: number },
    ) =>
  ...
  reactions: {
    ...
    users: (messageId: string, emoji: string) =>
      request<Array<{ id: string; display_name: string; avatar_color: string }>>(
  ...
  reminders: {
    ...
    create: (messageId: string, remindAt: string) =>
      request<Reminder>(`/messages/${messageId}/remind`, {
        method: 'POST',
  ...
  scheduledMessages: {
    list: (channelId: string) =>
      request<ScheduledMessage[]>(`/channels/${channelId}/scheduled-messages`),
  ...
  polls: {
    ...
    voters: (pollId: string, answerId: string) =>
      request<
        Array<{ id: string; display_name: string; avatar_color: string; avatar_url?: string }>
  ...
  stickers: {
    ...
    create: (
      ...
      request<{ id: string }>(`/guilds/${guildId}/stickers`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  emojis: {
    ...
    create: (guildId: string, input: { name: string; url: string; animated?: boolean }) =>
      request<{ id: string }>(`/guilds/${guildId}/emojis`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  sounds: {
    ...
    create: (
      ...
      request<{ id: string; file_url: string }>(`/guilds/${guildId}/sounds`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  commands: {
    ...
    create: (guildId: string, input: { name: string; description: string; response: string }) =>
      request<{ id: string }>(`/guilds/${guildId}/commands`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
    ...
    run: (channelId: string, name: string, args?: Record<string, string>) =>
      request<Message>(`/channels/${channelId}/commands/run?name=${encodeURIComponent(name)}`, {
        method: 'POST',
  ...
  threads: {
    create: (
      ...
      request<Channel>(`/channels/${channelId}/threads`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  stageInstances: {
    create: (input: {
      ...
      request<{ channel_id: string; topic: string }>('/stage-instances', {
        method: 'POST',
        body: JSON.stringify(input),
      }),
    get: (channelId: string) =>
      request<{ channel_id: string; topic: string; started_by: string; started_at: string }>(
  ...
  events: {
    ...
    create: (
      ...
      request<{ id: string }>(`/guilds/${guildId}/events`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  follows: {
    follow: (channelId: string, targetChannelId: string) =>
      request<any>(`/channels/${channelId}/followers`, {
        method: 'POST',
  ...
  reactionRoles: {
    ...
    create: (
      ...
      request<any>(`/guilds/${guildId}/reaction-roles`, {
        method: 'POST',
        body: JSON.stringify(input),
      }),
  ...
  welcome: {
    ...
    update: (guildId: string, input: any) =>
      request<any>(`/guilds/${guildId}/welcome`, { method: 'PATCH', body: JSON.stringify(input) }),
