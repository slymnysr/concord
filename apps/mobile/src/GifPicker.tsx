...
import { View, TextInput, FlatList, TouchableOpacity, Image, StyleSheet, Modal, Pressable, ActivityIndicator } from 'react-native';
import { colors } from './theme';

const GIPHY_KEY = 'dc6zaTOxFJmzC';

export function GifPicker({ visible, onPick, onClose }: { visible: boolean; onPick: (url: string) => void; onClose: () => void }) {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={s.backdrop} onPress={onClose}>
        <Pressable style={s.sheet} onPress={() => {}}>
          <TextInput style={s.search} value={q} onChangeText={setQ} placeholder="GIF ara…" placeholderTextColor={colors.inkTertiary} autoFocus />
          {loading ? (
...
const s = StyleSheet.create({
  ...
  search: { backgroundColor: colors.surface2, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9, color: colors.ink, marginBottom: 10 },
