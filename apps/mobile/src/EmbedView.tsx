...
export function EmbedView({ messageId }: { messageId: string }) {
  ...
  useEffect(() => {
    let c = false;
    api.messages.embeds(messageId).then((e) => { if (!c) setEmbeds(e); }).catch(() => {});
    return () => { c = true; };
  }, [messageId]);
  ...
  return (
    <View>
      {embeds.map((e) => (
        <TouchableOpacity key={e.id} style={s.card} onPress={() => Linking.openURL(e.url).catch(() => {})}>
          {!!e.site_name && <Text style={s.site}>{e.site_name}</Text>}
          {!!e.title && <Text style={s.title}>{e.title}</Text>}
          {!!e.description && <Text style={s.desc} numberOfLines={3}>{e.description}</Text>}
          {!!e.image_url && <Image source={{ uri: e.image_url }} style={s.img} resizeMode="cover" />}
        </TouchableOpacity>
      ))}
    </View>
  );
...
const s = StyleSheet.create({
  card: { backgroundColor: colors.surface2, borderLeftWidth: 3, borderLeftColor: colors.brand, borderRadius: 8, padding: 10, marginTop: 6 },
  ...
  img: { width: '100%', height: 150, borderRadius: 6, marginTop: 8, backgroundColor: colors.surface3 },
});
