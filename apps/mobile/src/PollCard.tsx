...
export function PollCard({ messageId, meId }: { messageId: string; meId: string }) {
  ...
  const reload = () => api.polls.forMessage(messageId).then(setPoll).catch(() => {});
  ...
  return (
    <View style={s.card}>
      ...
      {poll.answers.map((a) => {
        ...
        return (
          <TouchableOpacity key={a.id} style={[s.ans, a.me_voted && s.ansVoted]} disabled={poll.expired} onPress={() => vote(a.id, a.me_voted)}>
            <View style={[s.bar, { width: `${pct}%` }]} />
            <Text style={s.ansText}>{a.emoji ? a.emoji + ' ' : ''}{a.answer_text}{a.me_voted ? '  ✓' : ''}</Text>
            <Text style={s.pct}>{a.count} · {pct}%</Text>
          </TouchableOpacity>
        );
      })}
      <View style={s.foot}>
        <Text style={s.total}>{total} oy{poll.expired ? ' · kapandı' : ''}</Text>
        {!poll.expired && poll.created_by === meId && (
          <TouchableOpacity onPress={async () => { try { await api.polls.close(poll!.id); reload(); } catch {} }}>
...
const s = StyleSheet.create({
  card: { backgroundColor: colors.surface2, borderRadius: 12, padding: 12, marginTop: 6, borderWidth: 1, borderColor: colors.line },
  q: { color: colors.ink, fontSize: 15, fontWeight: '800', marginBottom: 10 },
  ans: { position: 'relative', overflow: 'hidden', borderRadius: 9, backgroundColor: colors.surface3, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 6, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
