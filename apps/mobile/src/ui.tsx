...
export function Row({ label, value, onPress, danger, right }: {
  label: string; value?: string; onPress?: () => void; danger?: boolean; right?: ReactNode;
}) {
  ...
  return (
    <Cmp style={ui.row} onPress={onPress}>
      ...
      {right ?? (value !== undefined ? <Text style={ui.rowValue} numberOfLines={1}>{value}</Text> : onPress ? <Text style={ui.chevron}>›</Text> : null)}
    </Cmp>
  );
}

export function Avatar({ name, color, url, size = 40 }: { name?: string; color?: string; url?: string; size?: number }) {
  return (
    <View style={[ui.avatar, { width: size, height: size, borderRadius: size / 2, backgroundColor: color || colors.surface3 }]}>
      <Text style={[ui.avatarText, { fontSize: size * 0.42 }]}>{(name || '?').slice(0, 1).toUpperCase()}</Text>
    </View>
  );
}
...
export function InputModal({
  visible, title, placeholder, initial, secure, multiline, submitLabel, onCancel, onSubmit,
}: {
  visible: boolean; title: string; placeholder?: string; initial?: string;
  secure?: boolean; multiline?: boolean; submitLabel?: string;
...
export const ui = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 1, borderColor: colors.line, gap: 4,
  },
  ...
  sectionTitle: { color: colors.inkTertiary, fontSize: 11, fontWeight: '800', marginBottom: 8, marginLeft: 4, letterSpacing: 0.5 },
  sectionCard: { backgroundColor: colors.surface1, borderRadius: 14, overflow: 'hidden' },
  row: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingHorizontal: 14, paddingVertical: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: colors.line, gap: 10,
  },
  ...
  input: {
    ...
    paddingHorizontal: 14, paddingVertical: 11, color: colors.ink, fontSize: 15,
  },
  btn: { backgroundColor: colors.brand, borderRadius: 12, paddingVertical: 13, alignItems: 'center' },
  btnText: { color: '#06281F', fontWeight: '800', fontSize: 15 },
  btnGhost: { backgroundColor: colors.surface2, borderRadius: 12, paddingVertical: 13, alignItems: 'center' },
  btnGhostText: { color: colors.ink, fontWeight: '700', fontSize: 15 },
  headerBtn: { color: colors.brand, fontSize: 15, fontWeight: '700', paddingHorizontal: 8, paddingVertical: 4 },
