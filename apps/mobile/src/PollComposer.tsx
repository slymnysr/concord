...
export function PollComposer({ visible, onCancel, onCreate }: {
  visible: boolean; onCancel: () => void; onCreate: (question: string, answers: string[]) => void;
}) {
  const [question, setQuestion] = useState('');
  const [answers, setAnswers] = useState(['', '']);

  useEffect(() => { if (visible) { setQuestion(''); setAnswers(['', '']); } }, [visible]);
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onCancel}>
      <Pressable style={s.backdrop} onPress={onCancel}>
        <Pressable style={s.sheet} onPress={() => {}}>
          ...
          <TextInput style={ui.input} value={question} onChangeText={setQuestion} placeholder="Soru" placeholderTextColor={colors.inkTertiary} />
          {answers.map((a, i) => (
            <TextInput
              key={i}
          ...
          <View style={s.btns}>
            <TouchableOpacity style={[ui.btnGhost, { flex: 1 }]} onPress={onCancel}><Text style={ui.btnGhostText}>Vazgeç</Text></TouchableOpacity>
            <TouchableOpacity style={[ui.btn, { flex: 1 }, !valid && { opacity: 0.4 }]} disabled={!valid} onPress={() => onCreate(question.trim(), answers.map((a) => a.trim()).filter(Boolean))}>
