...
export function EmojiPicker({ visible, onPick, onClose }: {
  ...
  useEffect(() => {
    AsyncStorage.getItem(RECENT_KEY).then((v) => { if (v) try { setRecents(JSON.parse(v)); } catch {} }).catch(() => {});
  }, []);
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={s.backdrop} onPress={onClose}>
        <Pressable style={s.sheet} onPress={() => {}}>
          ...
          <TextInput style={s.search} value={q} onChangeText={setQ} placeholder="Ara…" placeholderTextColor={colors.inkTertiary} />
          <FlatList
            data={data}
